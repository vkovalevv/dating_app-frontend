...
function ProfileForm() {
  ...
  const [notification, setNotification] = useState(null)  // null | 'photo_success' | 'update_success' | 'error'
  ...
  const handleSubmit = async (e) => {
    ...
    api.patch('/users/me/update-info', user)
      .then((res) => { console.log(res) 
        setNotification('update_success')
    ...
    <div className='flex w-full h-screen items-center justify-center '>
      ...
      {user &&
        <div className="flex gap-8 items-stretch">
          <Carousel imagesList={user.images} />
          <form
            className="flex flex-col gap-4 w-full max-w-md mx-auto bg-gray-300 rounded-xl shadow-sm p-6"
            onSubmit={handleSubmit}
          >
            {notification === 'photo_success' && <p className="text-green-600 text-sm">Photo successfully uploaded!</p>}
            {notification === 'error' && <p className="text-red-600 text-sm">{errorMessage}</p>}
            {notification === 'update_success' && <p className="text-green-600 text-sm">Profile successfully updated!</p>}
            <p>{user.email}</p>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input className="border rounded px-3 py-2 w-full" type="text" id="user.description" value={user.description ?? ''} onChange={e => setUser({ ...user, description: e.target.value })} />
            </div>
